...
/**
 * MyiuLib's locale API. It can be used for easy localization; you
...
public class Locale {
	 ...
	 * MyiuLib attempts to load locales first from the "locales"
	...
	public Locale(String plugin) {
		this.plugin = plugin;
		prefix = plugin.equals("MyiuLib") ? null : "[" + plugin + "]";
	}
		...
		if (Bukkit.getPluginManager().getPlugin(plugin).getClass().getClassLoader().getResource("locales") != null) {
			...
			try {
				if (is != null) {
					...
					if (loc != null) {
						MyiuLib.log(this.getMessage("plugin.event.loaded-locale", loc), LogLevel.INFO);
					}
				}
				if (defaultIs != null) {
					if (!legacy) {
